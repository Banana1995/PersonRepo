...
- 类型通配符表示一个未知的类型。
  ...
  > `? super Apple`表示参数类型是`Apple`父类的泛型，可以往容器内写入，不能读取。因为可以用任意的下边界Apple及其子类来调用写入的方法；但是当读取的时候，并不能将父类转换为子类。
  >
  > `? extend Apple`表示参数类型是`Apple`子类的泛型，可以从容器中读取，不能写入。因为读取的方法可以转换为父类`Apple`，但是不能使用父类`Apple`来调用写入的方法。

- **`List<Apple>`是原始类型`List`的子类**
